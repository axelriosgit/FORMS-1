...
public partial class Form1 : Form
{
    ...
    // EVENTO PARA COLOREAR FILAS DINÁMICAMENTE
    private void dgvProductos_CellFormatting(object? sender, DataGridViewCellFormattingEventArgs e)
    {
        if (e.RowIndex >= 0 && dgvProductos.Rows[e.RowIndex].DataBoundItem is Producto producto)
        {
            ...
            if (e.CellStyle != null)
            {
                ...
                e.CellStyle.SelectionBackColor = colorFondo;
    ...
    private bool ValidarCampos(out string nombre, out decimal precio, out int stock, out string rubro)
    {
        ...
        if (precio <= 0)
        {
            MessageBox.Show("El precio debe ser mayor al valor 0.", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            nudPrecio.Focus();
            return false;
        }
    ...
    private void btnEditar_Click(object sender, EventArgs e)
    {
        ...
        if (producto == null)
        {
            MessageBox.Show("Seleccione un producto para editar.", "Editar", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }
        ...
        if (indiceRubro >= 0)
        {
            txtRubro.SelectedIndex = indiceRubro;
        }
        else
        {
            txtRubro.SelectedIndex = -1;
    ...
    {
        if (!int.TryParse(txtIdEliminar.Text.Trim(), out int id))
        {
            MessageBox.Show("Ingrese un ID válido.", "Eliminar", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }

        Producto? producto = _controller.ObtenerTodos().Find(p => p.Id == id);
        ...
        DialogResult confirmar = MessageBox.Show($"¿Está seguro de eliminar el producto \"{producto.Nombre}\"?", "Confirmar eliminación", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
    ...
    private void btnEliminarStock_Click(object sender, EventArgs e)
    {
        ...
        DialogResult confirmar = MessageBox.Show($"Se eliminarán {cantidad} producto(s) con stock menor o igual a {limite}. ¿Desea continuar?", "Confirmar eliminación", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
    ...
    private void btnExportar_Click(object sender, EventArgs e)
    {
        ...
        lineas.Add("==============================================================");
        lineas.Add("                    GESTOR DE PRODUCTOS");
        lineas.Add("==============================================================");
        lineas.Add("");
        lineas.Add(string.Format("{0,-5} | {1,-25} | {2,12} | {3,8} | {4,-20}", "ID", "NOMBRE", "PRECIO", "STOCK", "RUBRO"));
        lineas.Add("------+---------------------------+--------------+----------+---------------------");

        foreach (Producto producto in lista)
        {
            lineas.Add(string.Format("{0,-5} | {1,-25} | {2,12:N2} | {3,8} | {4,-20}",
                producto.Id,
